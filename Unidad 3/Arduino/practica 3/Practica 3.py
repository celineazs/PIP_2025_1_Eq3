...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    # Área de los slots
    def control(self):
        texto = self.btn_control.text()  # Obtener texto actual
        if texto == "PRENDER":
            self.btn_control.setText("APAGAR")
            self.arduino.write("1".encode())  # Se corrigió strip por write
        else:
            ...
            if self.arduino.inWaiting():
                ...
                if cadena != "":
                    ...
                    if self.bandera == 0:
                        ...
                        self.lista_datos.addItem(str(cadena[0]))
                        self.lista_datos.setCurrentRow(self.lista_datos.count() - 1)

                        self.lista_datos_2.addItem(str(cadena[1]))
                        self.lista_datos_2.setCurrentRow(self.lista_datos.count() -1)

                        self.lista_datos_3.addItem(str(cadena[2]))
                        self.lista_datos_3.setCurrentRow(self.lista_datos.count() - 1)
